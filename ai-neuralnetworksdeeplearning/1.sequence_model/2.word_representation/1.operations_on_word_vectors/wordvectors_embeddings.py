...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = np.sqrt(np.abs(1-np.sum(np.square(mu_orth)))) * ((e_w1B-mu_B)/(np.sqrt(np.sum(np.square((e_w1-mu_orth)-mu_B)))))
    corrected_e_w2B = np.sqrt(np.abs(1-np.sum(np.square(mu_orth)))) * ((e_w2B-mu_B)/(np.sqrt(np.sum(np.square((e_w2-mu_orth)-mu_B)))))
...
#word_list = ['lipstick', 'guns', 'science', 'arts', 'literature', 'warrior','doct','technology', 'fashion', 'teacher', 'engineer', 'pilot', 'computer' ]
word_list = ['lipstick', 'guns', 'science', 'arts', 'literature', 'warrior','technology', 'fashion', 'teacher', 'engineer', 'pilot', 'computer', 'singer']
for w in word_list:
    print (w, cosine_similarity(word_to_vec_map[w], g))


#4.) Neutralize bias for non-gender specific words
e = "receptionist"
